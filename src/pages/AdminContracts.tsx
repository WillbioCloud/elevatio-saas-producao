...
interface ContractSignatureRow { contract_id: string | null; status: 'pending' | 'signed' | 'rejected' | null; }
interface ContractSignatureSummary { signatures_count: number; pending_signatures_count: number; signed_signatures_count: number; rejected_signatures_count: number; }
...
export default function AdminContracts() {
  ...
  // A LÓGICA ORIGINAL E BLINDADA DE BUSCA (NÃO ALTERADA)
  const fetchContracts = async () => {
    ...
    if (contractsError || !contractsData) { setLoading(false); return; }
  ...
  useEffect(() => { fetchContracts(); }, [user?.company_id]);
  ...
  const filteredContracts = contracts.filter(c => {
    ...
    const matchesSearch = (c.properties?.title || '').toLowerCase().includes(searchLow) ||
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-in fade-in pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard label="Contratos Ativos" value={metrics.active} icon={<Icons.CheckCircle2 size={20} />} color="emerald" />
        <MetricCard label="Aguardando Assinatura" value={metrics.pending} icon={<Icons.Clock size={20} />} color="amber" />
        <MetricCard label="Valor Total Ativo" value={new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL', maximumFractionDigits: 0 }).format(metrics.totalValue)} icon={<Icons.DollarSign size={20} />} color="blue" />
      </div>
        ...
        <div className="p-4 border-b border-slate-100 dark:border-slate-800/60 flex flex-col md:flex-row gap-4 justify-between items-center bg-white/50 dark:bg-slate-900/50">
          <div className="flex gap-1.5 p-1 bg-slate-100/80 dark:bg-slate-800/80 rounded-xl overflow-x-auto w-full md:w-auto custom-scrollbar">
            {['all', 'sale', 'rent', 'administrative', 'signatures'].map(type => (
              ...
              >
                {type === 'all' ? 'Todos' : type === 'sale' ? 'Venda' : type === 'rent' ? 'Locação' : type === 'administrative' ? 'Administrativo' : 'Assinaturas'}
              </button>
            ))}
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[850px]">
            <thead>
              <tr className="bg-slate-50/80 dark:bg-slate-800/50 text-[11px] uppercase tracking-wider text-slate-500 font-bold border-b border-slate-100 dark:border-slate-800/60">
                ...
                <th className="p-4 hidden md:table-cell text-center">Emissão</th>
