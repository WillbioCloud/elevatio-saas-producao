...
export default function AdminContracts() {
  ...
  const filteredContracts = contracts.filter((c) => {
    ...
    const matchesSearch =
      (c.property?.title || '').toLowerCase().includes(searchLow) ||
      (c.lead?.name || '').toLowerCase().includes(searchLow);

    if (filterType === 'signatures') return matchesSearch && (c.status === 'pending' || c.status === 'draft');
            ...
            <>
              ...
              <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-100 dark:border-slate-800 z-20 overflow-hidden animate-in fade-in slide-in-from-top-2">
                ...
                <button
                  ...
                  className="w-full flex items-center gap-3 px-4 py-3 text-sm text-left hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
                >
                  <div className="p-1.5 bg-slate-100 text-slate-600 rounded-lg dark:bg-slate-800">
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center">
                    <Icons.Loader2 className="animate-spin mx-auto text-brand-500 mb-2" /> Carregando contratos...
                  </td>
                </tr>
